"""Skill-targeted research E2E: Implementation -> Review -> research -> replan.

SF-32 (plan ref SF-033, SF-A-4 §14-C): the assumption-failure loop exercised
as one scenario through a skill-targeted research Run. It runs on the frozen
``tests/fixtures/workflows/runtime-reference/software-change.yaml`` (SF-42);
the product reference workflow routes to the research step instead, which
``test_e2e_fundamental_assumption.py`` covers. Every Task starts at
...
def test_skill_targeted_research_loop_to_approved(ws, workflows):
